...
- **KOPAW**（Kongar Pipe Audio/Video Wire）— 音视频管线，第一优先级，对标 PipeWire：
  ...
  支持插件 ABI、网络输入、转码封装、硬件解码回退与 **Vulkan 图像导出 DMA-BUF 零拷贝**。
...
| P2.1/P2.3/P2.4 | 插件 ABI、网络输入/转码、lavfi 节点 | ✅ |
...
| 后续 | 多入边/工作窃取调度、硬解原生导出、NV12 导入、swapchain 流水化、真机 KMS 直出 | ⬜ |
...
`--duration SEC` · `--filter video:<链>|audio:<链>` · `--kopms-bus NAME`（视频零拷贝上屏）。
环境变量：`KOP_LOG=debug|info|warn|error` · `KOPAW_HWACCEL=auto|none|vaapi|cuda` ·
`KOPAW_STATS=1`（每 5s 性能基线）· `KOPAW_SCHED=pool`（工作窃取池调度）。
...
kopaw/
  abi/include/        kopaw_abi.h（cbindgen 生成，勿手改；当前 5.1）
...
2. **ABI 头只由 cbindgen 生成**（当前 5.1），函数指针类型必须内联书写；
...
- 硬解帧经 SDK 的 Vulkan 导出路径为零拷贝；解码器原生 DMA-BUF 导出（VAAPI/CUVID）待接入；
- 多平面 YUV（NV12）导入、dmabuf v4 feedback、真机 KMS 直出验证属后续里程碑。
